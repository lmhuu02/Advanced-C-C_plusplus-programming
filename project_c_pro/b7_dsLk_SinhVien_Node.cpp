...
void insert(list &L, sinhvien sv)
{
    node *p;    /*khai bao con tro p kieu node*/
    p = MakeNode(p,sv);  /*tao node p*/
...
void insert_k(list &L, sinhvien sv, int k)
{
    node *p, *q = L;
    int i = 1;
...
void input(list &L)
{
    ...
    do{
        ...
        if(strcmp(sv.msv,"ok") != 0)
        {
            insert_k(L,sv,len(L)+1);
        }else{
            break;
        }
    }while(strcmp(sv.msv,"ok") != 0);
}
/*output data*/
void output(list L)
{
    ...
    while(p != NULL)
    {
        printf("\tma sv: %s\tho ten: %s\t tuoi: %d\n ",p->sv.msv, p->sv.hoten,p->sv.tuoi);
        p = p->next;
    }
}
